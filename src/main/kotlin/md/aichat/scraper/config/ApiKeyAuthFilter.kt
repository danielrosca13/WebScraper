...
import md.aichat.scraper.service.ApiKeyService
    ...
    override fun doFilterInternal(
        ...
        if (isSwagger) {
            ...
            if (username != swaggerUsername || password != swaggerPassword) {
                logger.warn("Invalid Swagger credentials: username={$username}, password=****")
                response.status = HttpServletResponse.SC_UNAUTHORIZED
        ...
        if (!apiKeyService.validateApiKey(requestApiKey)) {
